...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, default='../AutoGL/configs/nodeclf_nas_macro_benchmark2.yml')
    parser.add_argument('--dataset', choices=['cora', 'citeseer', 'pubmed'], default='cora', type=str)

    args = parser.parse_args()

    dataset = build_dataset_from_name(args.dataset)
    label = dataset[0].nodes.data["label"][dataset[0].nodes.data["test_mask"]].cpu().numpy()
